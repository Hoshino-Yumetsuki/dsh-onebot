...
export const Config: z<OneBotConfig> = z.object({
  transport: z
    .union(['forward-ws', 'reverse-ws', 'http'])
    .default('forward-ws')
    .description('传输模式'),
  url: z
    .string()
    .default('ws://127.0.0.1:3001')
    .description('正向 WebSocket 地址或 HTTP API 基础地址'),
  listenHost: z
    .string()
    .default('127.0.0.1')
    .description('反向 WebSocket 或 HTTP webhook 监听地址'),
  ...
  webhookSecretRef: z
    .string()
    .default('ONEBOT_WEBHOOK_SECRET')
    .description('webhook 签名密钥的 DSH 凭据引用'),
  ...
  requestTimeout: z
    ...
    .min(1)
    .step(1)
    .description('OneBot API 请求超时（毫秒）'),
  heartbeatTimeout: z
    .number()
    .default(120000)
    .min(0)
    .step(1)
    .description('心跳超时（毫秒，0 表示禁用）'),
  ...
  userAccessMode: z
    .union(['disabled', 'allowlist', 'blocklist'])
    .default('disabled')
    .description('用户访问控制模式'),
